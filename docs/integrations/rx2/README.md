...
obs
//ObservableKW(observable=io.reactivex.internal.operators.observable.ObservableFromArray@603a0566)
```

```kotlin
val flow = Flowable.fromArray(1, 2, 3, 4, 5).k()
flow
//FlowableKW(flowable=io.reactivex.internal.operators.flowable.FlowableFromArray@2648b5aa)
```

```kotlin
val subject = PublishSubject.create<Int>().k()
subject
//ObservableKW(observable=io.reactivex.subjects.PublishSubject@5bc59c43)
```
...
```kotlin
obs.value()
//io.reactivex.internal.operators.observable.ObservableFromArray@603a0566
```

```kotlin
flow.value()
//io.reactivex.internal.operators.flowable.FlowableFromArray@2648b5aa
```

```kotlin
subject.value()
//io.reactivex.subjects.PublishSubject@5bc59c43
```
